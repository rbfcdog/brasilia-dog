...
export default function HistoryPage() {
  return (
    <PageFrame
      ...
      actions={<button className="inline-flex h-10 items-center gap-2 rounded-lg border border-line bg-white px-4 font-mono text-[10px] uppercase tracking-[0.1em]"><Download className="size-3.5" /> Export</button>}
    >
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-line bg-ink p-5 text-white shadow-sm md:col-span-2">
          <p className="font-mono text-[9px] uppercase tracking-[0.14em] text-white/50">Approved spend · August</p>
          <div className="mt-4 flex items-end justify-between gap-4">
            <p className="font-mono text-3xl font-semibold">$331.33</p>
            <span className="flex items-center gap-1 rounded-full bg-success px-2.5 py-1 font-mono text-[9px] text-success-ink"><TrendingDown className="size-3" /> 12% below limit</span>
          </div>
          <div className="mt-7 flex h-16 items-end gap-2" aria-label="Illustrative spending chart">
            {[30, 46, 25, 62, 42, 78, 54, 88, 66, 96, 71, 58].map((height, index) => (
              <span key={index} className="flex-1 rounded-t-sm bg-white/15 last:bg-success" style={{ height: `${height}%` }} />
            ))}
          </div>
        </div>
      ...
      <div className="mt-4 overflow-hidden rounded-2xl border border-line bg-white shadow-sm">
        <div className="flex items-center gap-2 border-b border-line px-5 py-4"><ReceiptText className="size-4 text-primary" /><h2 className="text-sm font-semibold">Recent activity</h2></div>
            ...
            <div key={transaction.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[1fr_auto] sm:items-center">
              <div className="flex items-start gap-3">
                <span className={`mt-0.5 grid size-8 place-items-center rounded-lg ${transaction.status === "Approved" ? "bg-success/35 text-success-ink" : "bg-danger-soft text-danger"}`}>
                  {transaction.status === "Approved" ? <Check className="size-4" /> : <CircleX className="size-4" />}
                </span>
              ...
              <div className="flex items-center justify-between gap-5 pl-11 sm:pl-0">
                ...
                <ArrowDownRight className="size-4 text-muted" />
              </div>
            </div>
          ))}
        </div>
