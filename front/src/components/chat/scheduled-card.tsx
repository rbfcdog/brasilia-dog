import { ArrowRight, CalendarClock, CreditCard, Radar } from "lucide-react";
  ...
  return (
    <article className="max-w-xl rounded-2xl border border-primary/15 bg-primary-soft/60 p-5 shadow-sm sm:ml-10">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 font-mono text-[10px] uppercase text-subtle">
        <span className="flex items-center gap-1.5"><CalendarClock className="size-3.5" aria-hidden="true" /> {purchase.validityHours} hours remaining</span>
        <span>Max ${purchase.maximumAmount.toFixed(2)}</span>
        <span className="flex items-center gap-1.5"><CreditCard className="size-3.5" aria-hidden="true" /> {purchase.paymentMethod.brand} •••• {purchase.paymentMethod.last4}</span>
      </div>
